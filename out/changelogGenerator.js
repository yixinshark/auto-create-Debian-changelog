...
class ChangelogGenerator {
    ...
    async generateBasicEntry() {
        ...
        if (!author || !email) {
            const configure = await vscode.window.showWarningMessage('请先配置作者信息', '立即配置');
            if (configure) {
     ...
     */
    formatEntry(entry) {
        const dateStr = (0, moment_1.default)(entry.date).format('ddd, DD MMM YYYY HH:mm:ss ZZ');
     ...
     * 从提交信息生成变更列表
     */
    generateChangesFromCommits(commits) {
        const changes = [];
     ...
     * 用Issue信息增强提交消息
     */
    enhanceMessageWithIssue(message, issue) {
        // 如果消息过于简单，用Issue标题替换
     ...
     */
    async getPackageNameFromChangelog(workspaceRoot) {
     ...
     */
    async getPackageNamesFromControl(workspaceRoot) {
