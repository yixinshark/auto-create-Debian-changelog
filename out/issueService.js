...
class IssueService {
     ...
     * 为提交信息添加Issue详情
     */
    async enrichCommitsWithIssues(commits) {
        // 检查是否启用Issue详情功能
        ...
        try {
            ...
            // 批量获取Issue详情
            const issueDetails = await this.fetchIssueDetails(Array.from(allIssueRefs), repoType, repoInfo);
            // 将Issue详情添加到对应的提交中
            for (const commit of commits) {
     ...
     * 批量获取Issue详情
     */
    async fetchIssueDetails(issueRefs, repoType, repoInfo) {
     ...
     * 获取GitHub Issue详情
     */
    async fetchGitHubIssues(issueRefs, repoInfo, results) {
     ...
     * 获取GitLab Issue详情
     */
    async fetchGitLabIssues(issueRefs, repoInfo, results) {
     ...
     * 显示API配置帮助
     */
    async showApiConfigurationHelp() {
        const message = `
...
配置完成后，插件将能够自动获取Issue的详细信息并生成更有意义的changelog条目。
        `;
        const action = await vscode.window.showInformationMessage('Issue详情获取功能需要API令牌配置', '查看帮助', '立即配置');
        if (action === '查看帮助') {
            const panel = vscode.window.createWebviewPanel('api-help', 'API配置帮助', vscode.ViewColumn.One, {});
            panel.webview.html = `
                <html>
